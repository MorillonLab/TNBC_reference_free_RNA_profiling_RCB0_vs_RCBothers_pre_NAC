...
# USAGE EXAMPLE
# bash split_cv.sh -s /store/EQUIPES/SSFA/hugues.herrmann/data_melanoma/predictive_models/metadata/matrice_design_resp_vs_prog.tsv -n 4 -o /store/EQUIPES/SSFA/hugues.herrmann/data_melanoma/predictive_models/matrices_cv/

NOCOLOR='\033[0m'
RED='\033[1;31m'

usage() { 
	echo -e "Usage: $0 <Required arguments>\nRequired arguments :\n\t-i\tcomplete sample condition file\n\t-k\tfold number for cross-validation\n\t-o\toutput directory\n" 1>&2
	exit
}

[[ $# -eq 0 ]] && usage

while getopts "i:k:o:s:" opt
do
	case $opt in
		i)	smp_condi_path=$OPTARG
			;;
		k)	nb_fold=$OPTARG
			;;
		o)	out_dir=$OPTARG
...
do
	shuf --random-source=<(get_seeded_random $user_seed) $fs | awk -v outd=$out_dir -v nfold=$nb_fold '{print $0 >> outd"/sampleshuf.tmp2."$2"_"NR%nfold".tsv"}'
done
